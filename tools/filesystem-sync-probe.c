...
/*
 * Bounded ext4 fdatasync probe for a disposable filesystem.
 *
 * The caller creates and mounts the isolated filesystem. This program refuses
 * non-ext4 targets, writes twelve one-mebibyte samples to one new file,
 * measures fdatasync(2), verifies every byte through O_DIRECT, removes the
 * file, and synchronizes the containing directory.
 */
...
#define PROBE_FILE ".mbp-ahci-filesystem-sync-probe"
